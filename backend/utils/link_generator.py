...
class LinkGenerator:
    ...
    def generate_linkedin_url(job_title: str, location: str = "Morocco") -> str:
        """Generate a direct LinkedIn search URL (avoids login splash when possible)"""
        encoded_title = quote(job_title)
        encoded_location = quote(location)
        # Direct search with filters; keeps users on job results, not signup landing
        return f"https://www.linkedin.com/jobs/search/?keywords={encoded_title}&location={encoded_location}&f_AL=true"
